...
function addRandomTile() {
    ...
    for(let i = 0; i < BOARD_SIZE; i++){
        for(let j = 0; j < BOARD_SIZE; j++)if (board[i][j] === 0) emptyCells.push({
            x: i,
            y: j
        });
    }
...
function startGame() {
    ...
    for(let i = 0; i < BOARD_SIZE; i++)for(let j = 0; j < BOARD_SIZE; j++)board[i][j] = 0;
    [
        ...cells
    ].forEach((cell)=>{
...
function moveVertical(direction) {
    ...
    for(let col = 0; col < BOARD_SIZE; col++){
        ...
        if (direction === "ArrowDown") currentColumn.reverse();
        for(let i = 0; i < BOARD_SIZE; i++)board[i][col] = currentColumn[i];
